---
title:  "AI aided look at healthy aging genes study"
...
if(sum(grepl(FN, list.files()))==0) stop("Check FN")
 ...
 figures_DIR <- file.path(WRKDIR,'Scripts', 'figures/M0A/')
 suppressMessages(dir.create(figures_DIR, recursive=T))
 opts_chunk$set(fig.path=figures_DIR)

 #tables_DIR <- file.path(WRKDIR,'Scripts', 'tables/M0A/')
...
<p><p/>
* In Section \@ref(sood-ai-review) we prompt **chatCPT** and **SciSpace** to provide a
technical review of Sood et al. (2015) [@Sood:2015aa].  Here,
as in most cases when dealing with esoteric material, the AI assistants get some
key concepts wrong while maintaining a steady tone of confidence.

<p><p/>
* In the next Section, \@ref(sood-ai-review-with-line-ref), we prompt
...
```{r m1a-study-design, fig.cap = CAPTION, include=T, fig.align='center', eval=F}
...
### Representativeness of Training Data
The study constructed the RNA classifier using a set of 297 samples drawn from different case-control cohorts (p. 7). The inclusion criteria aimed at age- and gender-matched controls to ensure comparability, and the authors later validated their findings on an expanded dataset of 717 samples (p. 7). However, while retrospective inclusion did not alter conclusions, the representativeness of these datasets with respect to broader populations remains uncertain. It is not explicitly demonstrated that the training data were a random subset of the target population (p. 12).

### Sample Size and Pooling Strategy
The classifier was developed using 150 probe-sets selected based on their high classification performance in leave-one-out cross-validation (LOOCV) (p. 3). The study does not clearly articulate how many independent data collection runs were aggregated for the final analysis. Some datasets were derived from GEO and others were from in-house experiments, but the merging strategies and potential biases introduced by pooling are not explicitly discussed (p. 12).

### Tissue Type Distribution Across Runs
The study evaluated multiple tissue types, including muscle, skin, and brain (p. 4). While independent external validation datasets were used, the distribution of these tissue types across experimental runs is unclear. The study does not report whether the proportion of different tissue types varied across batches, which could introduce batch-specific biases (p. 6).

### Association Between Experimental Run and Age
The study aims to classify samples based on age-related RNA signatures but does not explicitly evaluate whether experimental run effects correlate with sample age. Given that samples were sourced from different facilities using different platforms (e.g., Affymetrix vs. Illumina), it is possible that batch effects could confound biological age-related signatures (p. 5). The authors address some of these concerns by applying fRMA normalization (p. 12), yet explicit batch effect analyses (such as surrogate variable analysis) would strengthen conclusions.

### Data Normalization
The study applies quantile normalization using the LUMI package in R for Illumina data and fRMA for Affymetrix datasets (p. 13). While these methods are standard, their effectiveness in removing batch effects is not formally tested. The study notes that a small number of outlier arrays were removed based on hierarchical clustering (p. 14), but details on specific batch correction steps (such as ComBat or empirical Bayes approaches) are lacking.

### Classifier Validation and Generalizability
Classifier performance was assessed via LOOCV and independent dataset validation (p. 3). The area under the ROC curve (AUC) for various tissue types ranged from 0.78 to 0.99, indicating high classification accuracy (p. 4). Importantly, the authors validated the classifier using independent datasets, which supports its generalizability (p. 6). However, the robustness of the classifier in unseen, real-world clinical settings remains to be tested (p. 14).

### Conclusion
Overall, the statistical methodology employed in Sood et al. (2015) is rigorous but could be strengthened with more explicit analyses of batch effects, run-to-run variability, and sample representativeness. The classifier appears robust based on the reported validation, but further testing on prospective cohorts is needed to confirm its predictive performance in new populations (p. 15).
...
## Healthy Aging Genes Articles - Abstracts {#healthy-aging-genes-abs .tabset}

* There is no interpretation here - simply text extracted from abstracts.
...
diseases of older age. 
   - We take a transcriptomics approach to build
...
of cognitive health and prove that **a common healthy ageing gene-expression
program is detectable across multiple tissues**.

<br/>

#### Comment {-}

* From the abstract this appears to be an unconventional method which defies
the most rudimentary guidelines of good statistical practice: PQRS 

* Expand on PQRS

* When investigators believe that they have discovered something useful -
in this case some classifier with some accuracy - it may be hard to cast doubt 
on the results because of flaws in the method used to derive the results.  

* An alternative is to show that an equivalent result can be obtained
from a sham procedure.

* **Question - can any AI analysis tell us that the procedure in 
Sood  et al. (2014) is wacky?
...
# Individual Article Reviews  {.tabset}

```{r m1a-article-review, cache = T, cache.vars = "article_review_prompt_vec", fig.cap="Article review prompt",  results = "asis", eval=F}
...
```{r m1a-session-info, echo=T, eval=T}
 sessionInfo()
```
...
<!-- To run
# nohup Rscript -e "knitr::knit2html('_M0A-healthy_aging_genes.Rmd')" > _M0A-healthy_aging_genes.log  &

# Or
# nohup Rscript -e "rmarkdown::render('_M0A-healthy_aging_genes.Rmd')" > _M0A-healthy_aging_genes.log  &
