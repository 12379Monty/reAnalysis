---
title:  "Reference-Free Genomic Inference with SPLASH and OASIS - V2"
...
FN <- "_M2B-ref_free_genomic_inf_v2"
...
* We still need to better understand the statistical procedure used to "score"
samples for indenpendence, etc., but we can make some general remarks
which apply broadly, independent of the specifics of the statistical procedures.
...
should be ascertained empirically.
...
this is the case. 
<!--
   - if we are interested in considering the effect size in the identification proces, ...
...
the points falling in the tails may also not have rhe expected intrerpretration.
...
<p><p/>
* In this report we survey the scientific literature in order to
situate two papers from the [Salzman Lab](https://salzmanlab.stanford.edu/):
   - SPLASH: 
...
<p><p/>
* The report is divided into two parts, one focusing
...
<span style="color:blue">prior to `SPLASH`.

<p><p/>
* In Section \@ref(splash-ai-review) we prompt **chatGPT** and **scholar GPT** to provide a
technical review of Chaung et al. (2023).

<p><p/>
In Section \@ref(soa-test-contingency-tables), we use **Concensus** to obtain a
literature review summarizing the state of the art in contingency tables statistical tests	
<span style="color:blue">prior to `OASIS`</span>.


<p><p/>
* In Section \@ref(oasis-ai-review) we prompt **chatGPT** and **SciSpace** to provide a
technical review of [Baharav@Baharav:2024aa] (**OASIS**).


<p><p/>
* In Section \@ref(splash-citings) we use AI assisted research tools to summarize
...
Contingency tables are a cornerstone of statistical analysis for categorical data and are widely used in genomics, social sciences, and epidemiology. Numerous statistical tests have been developed to test for dependence between rows and columns. This review surveys the state-of-the-art methods with an emphasis on their statistical properties, particularly in the context of modern, high-dimensional and sparse applications.

1. Classical Approaches
1.1 Pearson’s Chi-Square Test
Pearson’s χ² test is one of the oldest and most widely used methods for testing independence in contingency tables. The test statistic is based on the sum of squared deviations between observed and expected counts:

mathematica
Copy
Edit
χ² = ∑ (Xᵢⱼ - Eᵢⱼ)² / Eᵢⱼ
Despite its popularity, the χ² test suffers from several known issues:

Finite-sample validity: The χ² approximation is unreliable when expected counts are small or sparse.
Lack of robustness: Sensitive to noise or overdispersion, common in biological data.
Poor interpretability: Provides no explanation for why the null is rejected.
1.2 Likelihood Ratio Test (G-Test)
The likelihood ratio test compares the likelihood of the data under the null and alternative models. It is sometimes more powerful than χ², but:

Still relies on asymptotic assumptions.
Shares interpretability issues with χ².
1.3 Fisher’s Exact Test
Fisher's exact test gives exact p-values for 2×2 tables using the hypergeometric distribution.

Exact, not approximate.
Not scalable to large or high-dimensional tables.
2. Simulation-Based and Resampling Methods
2.1 Permutation and MCMC-based Tests
These methods estimate the null distribution through repeated sampling:

Provide finite-sample valid p-values.
Computationally expensive, especially with multiple hypotheses or large tables.
Do not inherently offer interpretability.
2.2 Limited-Information Methods
These methods analyze selected subsets of table structure (e.g., univariate or bivariate margins):

Perform well empirically in some settings.
Rely on asymptotic approximations.
Not designed for arbitrary table geometries.
3. Modern Approaches for High-Dimensional Data
3.1 Correspondence Analysis
A visual and exploratory technique based on matrix decomposition (SVD of normalized residuals):

Helps identify latent structure in tables.
Not a statistical test: no p-values, no Type I error control.
Widely used for interpretation post χ².
3.2 SPLASH Framework
The SPLASH algorithm introduced by [Chaung et al., 2023][@Chaung:2023aa] redefines genomic inference as detecting sample-specific variation in k-mer distributions. Each k-mer anchor is tested via a contingency table.

Detects structured variation without a reference genome.
Computes a p-value bound for each anchor.
Enables robust inference in sparse sequencing datasets.
Motivates development of improved tests like OASIS.
4. OASIS: A Finite-Sample Valid Alternative
The OASIS method by [Baharav et al., 2024][@Baharav:2024] introduces a new family of statistical tests specifically designed for sparse, high-dimensional contingency tables, such as those used in SPLASH.

Key features:

✅ Finite-sample valid p-values using closed-form concentration bounds (e.g., Hoeffding's inequality).
✅ Interpretable: based on a linear test statistic 
𝑆
=
𝑓
𝑡
...
X
~
 c, allowing insight into table structure.
✅ More robust than χ² to noise and overdispersion.
✅ Computationally efficient: no need for permutations or sampling.
✅ Higher power in structured signal settings (e.g., two-group differences, time series).
This method addresses the key limitations of classical tests and has been validated in real applications like variant detection in SARS-CoV-2 and strain inference in M. tuberculosis.

5. Conclusion
Traditional tests like χ² and G-tests have well-known limitations in modern data settings. Resampling methods offer finite-sample validity but at high computational cost. Recent innovations such as SPLASH and OASIS provide powerful alternatives by focusing on finite-sample validity, statistical rigor, and interpretability — particularly critical in high-throughput genomic applications.
...
<!-- To run
## nohup Rscript -e "knitr::knit2html('_M2B-ref_free_genomic_inf_v2.Rmd')" > _M2B-ref_free_genomic_inf_v2.log  &

## Or
## nohup Rscript -e "rmarkdown::render('_M2B-ref_free_genomic_inf_v2.Rmd')" > _M2B-ref_free_genomic_inf_v2.log  &
