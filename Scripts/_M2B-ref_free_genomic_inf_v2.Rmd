...
* In the first section is a brief literature review of 
Statistical Tests for Contingency Table

<p><p/>
* In the next two sections  go over the data and analysis highlights
for the  SPLASH and OASIS.


In the rest of the report we use various  GPT tools to produce variosu reviews.
...
# Brief Literature Review: Statistical Tests for Contingency Tables
...
# SPLASH - Data and Analysis  {.tabset}
...
down from the anchers - is also compiled.
...
<!-- 
*  Under scenario 1.`
      - can differ from sample to sample and overall
      - differences are due to?
   - sample counts
      - differences are due to technical effects; library size, etc.
   - target counts = anchor * target

* Under independence: 
  - If we think of all the reads being sampled from
one universal pool, and reads are labeled with
anchor and target, we can think of the sample counts and the target
counts as random.
  - Independence:  
...
* $S_j$ is a measure in standard units up to a scaler ($\sigma$)  and
...
<span style="color:darkgreen">
* $S_j$ is a measure in standard units up to a scaler ($\sigma$)  and
* this works well as long as there are no high coverage targets with
different levels in different samples.
...
<p><p/>
<span style="color:darkgreen">
* $S_j$ values in equation \@ref(anchor-samples) are usually scaled
...
<span style="color:darkgreen">
*The effct size in Equation \@ref(#eq:effect-size) is expressed as a difference.  If the two groups have different sample sizes, the difference is not a good way to combine the 
two.
...
BELOW ARE SECTIONS ASSEMBLED by GPT 

***

# SPLASH: Reference-Free Genomic Inference {#ref-free-genomics  .tabset}
...
# OASIS: Test for Contingency Tables {#test-contingency-tables  .tabset}
...
**OASIS (Optimized Adaptive Statistic for Inferring Structure)** is a novel family of tests designed to overcome the limitations of traditional methods. Introduced by [@Baharav:2024], OASIS:
...
[@Baharav:2024]: Baharav, T. Z., Tse, D., & Salzman, J. (2024). OASIS: An interpretable, finite-sample valid alternative to Pearson’s X2 for scientific discovery. *PNAS*, 121(15), e2304671121. https://doi.org/10.1073/pnas.2304671121
