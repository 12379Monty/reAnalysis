...
FN <- "SPLASH_OASIS_summary"
if(sum(grepl(FN, list.files()))==0) stop("Check FN")

PREFIX <- "SPLASH_OASIS_summary" #- replace by FLOWCELL???
...
<p><p/>
* As a reference which data analysts might go to for guidance,
the articles lack the key pieces needed for the reader to get a true sense of 
performance and for the scope of the analysis necessary to obtain valid results.
...
unless the assumptions are true.  The crux of the work for the applied
statistician should be  to establish the null model.  In practice, the necessary
null model is almost always assumed without substantial supporting evidence.]
...
is suspect.  When we fit one prototype model to 20K, 50K or 100K feature data
...
other candidate discriminants (in addition to Pearson's $X^2$).
   - If p-values are to be taken at face value, their calibraion should be 
varidfied.
   - The p-value distribution an should also be examined to check for
...
context of contemporary analysus methods:
...
```{r SPLASH_OASIS_summary-write-bib, results="asis", echo=F}
...
<p><p/>
* [Part B - Specific Applications and Evidence](./_M4A-RUV/natureBiotechPaper/SPLASH_OASIS_summary
.html)
...
<!-- To run
## nohup Rscript -e "knitr::knit2html('SPLASH_OASIS_summary.Rmd')" > SPLASH_OASIS_summary.log  &
  
## Or
## nohup Rscript -e "rmarkdown::render('SPLASH_OASIS_summary.Rmd')" > SPLASH_OASIS_summary.log  &
