...
<p><p/>
* The goal of SPLASH is to detect sequence variation between the 
samples of a set - we interrogate the read counts in order
to distinghish two scenarios:  
   -  1. the reads for all samples were generated by a common source. 
eg the same biological units, or units from the same group.
...
<p><p/>
* To detect variation (in the sense of genomic sequence differences
between samples) a set of **anchors** - k-mers which appear among the reads -
are identified and combined into a set.
   - Computationally, it is easier to start with all possible k-mers and
count targets for each.


<p><p/>
* A set of **targets** - k-mers which appear among the reads at
locations a few loci down from the anchors - is also compiled.
...
<p><p/>
* Does this need to be cast into a statistical hypothesis testing problem?
   - One approch is to simply measure differences at the anchor level,
and subsequently investigate the biologiocal connection.

<p><p/>
* OASIS chooses to formulate the problem as one of the identification of anchors
for which the **conditional distributions of target counts
given the total number of targets** found for each sample anchor
are not independent.
   - The analysis is at the anchor level and the data are the
distribution of targets.
   - Modeling the counts as is done in the paper is assuming that the reads
containing each anchor are a randomly selected set of reads from a
population of anchor reads.
   - What is the population of reads from which the anchor reads are
sampled from?  The question "Are the results significant?" is not
a well-specified question until the previous questuion is answered.
...
$$\argmax_{c, f} \frac{f^{\top} \tilde{X} c}{\|f\|_2 \|c\|_2}$$
   - Finding the optimal 𝑓 is NP-hard for both formulations 
      - In practice: Use greedy algorithms (alternating maximization) 
      - Result: You get a local optimum, not the global optimum

   - Implication: **The “exact p-value” is for the 𝑓 you found**, not the optimal 𝑓. 
...
“train” and “test” datasets, constructs $f$ and $c$ that optimize
