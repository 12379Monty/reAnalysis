...
$nA \sym Multinommial(m, p_A)$ where $p_A =  {pa_1, pa_2, ..., pa_I}$ are the population
proportions of samples with $A == a_i$
      - Similarly $nB \sym Multinommial(m, p_B)$ where $p_B =  {pb_1, pb_2, ..., pb_I}$ are the population
proportions of samples with $B == b_j$
      - $if nAB = {\sum_{k=1}^m {A_k == a_r, B_k == b_c}, r = 1, ..., I, c = 1=, ..., J} = {nab_1_1, nab_1_2, ..., nab_I_J}
$nAB \sym Multinommial(m, p_AB)$ where $p_AB =  {pab_1_1, pab_1_2, ..., pab_I_J}$ are the population
...
distributions being multinomial - are direct results of simple random sampling.
   - In theory SRS is simple:  put the population ISs in a large urn, after stirring thoroughly, pick a ball,
note thr sample ID.  Repeat.
...
the population can be taken to be a very large set of reads from a
